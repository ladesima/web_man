    ...
    public function index(Request $request)
{
    ...
    $pesertas = $query->get()->map(function ($item) {
        ...
        if ($item->jalur === 'prestasi') {
            ...
            // 🔥 REGULER & AFIRMASI → hanya rapor
            if ($nilaiRapor !== null) {
                $nilaiTotal = $nilaiRapor;
            }
        }
    ...
    public function updateNilai(Request $request, $id)
{
    $request->validate([
        ...
        'nilai_prestasi' => 'nullable|numeric|min:0|max:100',
    ]);

    $pendaftaran = Pendaftaran::findOrFail($id);

    // 🔥 HITUNG TOTAL SESUAI JALUR
    if ($pendaftaran->jalur == 'prestasi') {
        $nilaiTotal = round(($request->nilai_rapor + $request->nilai_prestasi) / 2);
    } else {
        $nilaiTotal = $request->nilai_rapor;
    }
