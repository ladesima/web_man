...
$config = match ($status) {
    'perbaikan' => ['text'=>'Perlu Perbaikan','bg'=>'rgba(108,153,217,0.20)','color'=>'#1654AA','border'=>'#1654AA'],
    'lulus' => ['text'=>'Berkas Valid','bg'=>'rgba(34,197,94,0.15)','color'=>'#15803D','border'=>'#15803D'],
    'tidak_lulus' => ['text'=>'Berkas Ditolak','bg'=>'rgba(239,68,68,0.15)','color'=>'#DC2626','border'=>'#DC2626'],
    default => ['text'=>'Menunggu Verifikasi','bg'=>'rgba(14,165,233,0.15)','color'=>'#0284C7','border'=>'#0284C7'],
};
@endphp
...
<div class="flex justify-end gap-3 mb-5">
    ...
    <a href="{{ route('panitia.operasional.verifikasi.validasi', $pendaftaran->id) }}"
       ...
       style="background:#27C2DE; border-radius:4px;">
        <img src="{{ asset('ppdb/admin/operasional/verifikasiulang.png') }}" class="w-4 h-4">
        Verifikasi Ulang
    </a>
...
<div class="bg-white px-6 py-5 mb-4"
     style="border-radius:12px;border:1px solid #F3F3F3;box-shadow:0 4px 4px rgba(161,209,251,.25);">
    ...
    <div class="flex items-center">
        ...
        <div class="flex items-center gap-4 flex-1"
            ...
            <img src="{{ $pendaftaran->foto ? asset('storage/'.$pendaftaran->foto) : asset('ppdb/admin/operasional/cadangan.png') }}"
        ...
        <div class="text-right" style="padding-left:24px;">
            <p class="text-[12px] mb-1">No Pendaftaran:</p>
            <p class="text-[22px] font-bold">{{ $pendaftaran->id }}</p>
        </div>
...
<div class="col-span-3 bg-white px-6 py-5"
     style="border-radius:16px;border:1px solid #F3F3F3;box-shadow:0 4px 4px rgba(161,209,251,.25);">
    ...
    <div class="grid grid-cols-2 gap-x-8 gap-y-4">
        ...
        <div><p class="text-[11px]">Tempat, Tanggal Lahir</p><p class="text-[12px] font-medium">{{ $pendaftaran->ttl }}</p></div>
        <div><p class="text-[11px]">Pekerjaan Orang Tua</p><p class="text-[12px] font-medium">{{ $pendaftaran->pekerjaan_ortu }}</p></div>
        <div><p class="text-[11px]">Asal Sekolah</p><p class="text-[12px] font-medium">{{ $pendaftaran->asal_sekolah }}</p></div>
        <div><p class="text-[11px]">Penghasilan Orang Tua / Bulan</p><p class="text-[12px] font-medium">{{ $pendaftaran->penghasilan_ortu }}</p></div>
        <div><p class="text-[11px]">Alamat</p><p class="text-[12px] font-medium">{{ $pendaftaran->alamat }}</p></div>
        <div><p class="text-[11px]">Alamat Orang Tua</p><p class="text-[12px] font-medium">{{ $pendaftaran->alamat_ortu }}</p></div>
        <div><p class="text-[11px]">Nama Orang Tua</p><p class="text-[12px] font-medium">{{ $pendaftaran->nama_ortu }}</p></div>
        <div><p class="text-[11px]">Jumlah Bersaudara</p><p class="text-[12px] font-medium">{{ $pendaftaran->jumlah_saudara }}</p></div>
...
<h3 class="text-[13px] font-bold mb-1">Dokumen Unggahan</h3>
<div class="mb-4 border-b"></div>
...
<th class="text-left py-2 px-3 font-semibold">Dokumen</th>
...
@else
<span class="inline-block w-full text-center text-[11px] font-medium py-1"
style="background:#FEE2E2;color:#DC2626;border:1px solid #DC2626;border-radius:4px;">
Tidak Ada
</span>
@endif
</td>
...
<td class="py-2.5 px-3 text-center">
@if($file)
@php $url = route('preview.dokumen', ['file'=>$file]); @endphp
...
style="background: linear-gradient(135deg,#5EEAD4,#0284C7);border-radius:4px;">
...
<h3 class="text-[14px] font-semibold mb-2">Catatan Verifikasi</h3>
<div class="mb-4 border-b"></div>
...
<ul class="text-[13px] space-y-2 pl-5" style="list-style:disc;">

@if(!empty($verifikasi))
@foreach($verifikasi as $key=>$item)
@if(($item['status'] ?? '')=='no')
<li>{{ strtoupper(str_replace('_',' ',$key)) }} : {{ $item['catatan'] }}</li>
@endif
@endforeach
@endif

@if(empty($verifikasi) || collect($verifikasi)->where('status','no')->count()==0)
