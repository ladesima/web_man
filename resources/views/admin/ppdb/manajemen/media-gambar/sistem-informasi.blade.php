...
<div class="bg-white rounded-2xl p-8" style="box-shadow:0 2px 12px rgba(0,0,0,0.07);">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        @php
        $items = [
            ['name' => 'homepage',       'label' => 'Homepage',        'ukuran' => '1440x791'],
        ];
        @endphp

        @foreach($items as $item)
        @php
            $existing = $data[$item['name']] ?? null;
        @endphp

        <div>
            <label class="block text-[13px] font-semibold text-[#2B2A28] mb-1">
                {{ $item['label'] }}
                <span class="font-normal text-[11px] text-slate-400">
                    *ukuran {{ $item['ukuran'] }}
                </span>
            </label>
            ...
            <label id="box-{{ $item['name'] }}"
                ...
                {{-- IDLE --}}
                <div id="idle-{{ $item['name'] }}"
                     class="flex flex-col items-center {{ $existing ? 'hidden' : '' }}">
                ...
                {{-- DONE --}}
                <div id="done-{{ $item['name'] }}"
                     class="{{ $existing ? 'flex' : 'hidden' }} flex-col items-center">
